...
This is what the Workflow tool is for: one agent per package, the wave in parallel. Workflow needs Drew's explicit go-ahead each run — confirm it before calling, and never infer it from an earlier run.
